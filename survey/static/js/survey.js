...
function sendDataToServer(survey) {
    ...
    $.ajax({
        ...
        success: function(data, status) {
            // Let user know whether they qualify for screening
            if (data.qualifies) {
                $('#qualify_yes').removeClass('d-none');
            } else {
                $('#qualify_no').removeClass('d-none');
            }
...
// Validate if an email has already responded to survey
function validateEmailOnServer(_, { data, errors, complete }) {
    // Only validate email field
    if ('email' in data) {
        // Ajax GET request
        $.ajax({
            url: "/ajax/email_exists?email=" + data['email'],
            type: "GET",
            success: function(data, status) {
                // Set error message if email repeated
                if (data.exists === true) {
                    errors['email'] = 'Ya usted respondió a la encuesta.'
                }
                complete();
            },
            error: function(xhr, errmsg, err) {
...
$("#surveyContainer").Survey({
  ...
  onServerValidateQuestions: validateEmailOnServer
});